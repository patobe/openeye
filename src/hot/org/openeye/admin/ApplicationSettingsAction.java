...
public class ApplicationSettingsAction implements Serializable {
	...
	public void getSettings() {
		...
		if (!isUserCreated("admin")) {
			...
			if (!isRoleCreated("Customer")) {

				// Create role
				createRole("Customer", "Customer Role");

				log.info("Created Customer role.");
			}

			if (!isRoleCreated("Employee")) {

				// Create role
				createRole("Employee", "Employee Role");

				log.info("Created Employee role.");
			}

			if (!isRoleCreated("Vendor")) {

				// Create role
				createRole("Vendor", "Vendor Role");

				log.info("Created Vendor role.");
			}

			if (!isRoleCreated("Process Manager")) {

				// Create role
				createRole("Process Manager", "Process Manager Role");

				log.info("Created Process Manager role.");
			}

			if (!isRoleCreated("Report Manager")) {

				// Create role
				createRole("Report Manager", "Report Manager Role");

				log.info("Created Report Manager role.");
			}
		} else {
			// Reset login setting after reboot to allow user
			settings.setUserLogin(true);

			// Set default welcome message
			settings
					.setWelcomeMessage("Create an Open Eye user account from the registration link or by contacting your Open Eye support team.");
		}
